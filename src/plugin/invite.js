const invite = async (m, gss) => {
  try {
    ...
    if (!m.isGroup) return m.reply("*⚙️ GROUP වල command එකක්*");
    ...
    if (!isBotAdmins) {
      return m.reply('*⚙️ BOT MUST BE AN ADMIN TO USE THIS COMMAND.*');
    }

    if (!text) return m.reply(`*📤 INVITE කල යුතු number එක මේ ආකාරයට දන්න 📄අවදනයයි මෙම command බාවිතා නොකරන්න*\n\nExample:\n*${prefix + cmd}* 94761111111`);
    ...
    m.reply(`*✔️ AN INVITE LINK IS SENT TO THE USER.*`);
