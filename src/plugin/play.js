...
const playcommand = async (m, Matrix) => {
  ...
  if (validCommands.includes(cmd)) {
    if (!text) {
      return m.reply('song name එක ලබා දෙන්න🔍.');
    }
    ...
    try {
      ...
      const buttons = [
        ...
        },
        {
          "name": "quick_reply",
          "buttonParamsJson": JSON.stringify({
            display_text: "🎵 Audio Document එක",
...
YouTube Search\n\n*🔍Title:* ${currentResult.title}\n*✍️Author:* ${currentResult.author.name}\n*🥸 Views:* ${currentResult.views}\n*🏮 Duration:* ${currentResult.timestamp}\n`
    ...
    if (selectedId.startsWith('ඊ්ලග_')) {
      const nextIndex = parseInt(selectedId.replace('ඊලග_', ''));
      ...
      if (selectedMedia) {
        try {
          ...
          if (type === 'audio' && fileSizeInMB <= 300) {
            content = { audio: finalMediaBuffer, mimetype: 'audio/mpeg', caption: 'Downloaded by 𝗥𝗨𝗦𝗛-𝗠𝗗' };
          } else if (type === 'video' && fileSizeInMB <= 300) {
            content = { video: finalMediaBuffer, mimetype: 'video/mp4', caption: 'Downloaded by 𝗥𝗨𝗦𝗛-𝗠𝗗' };
            ...
            content = { document: finalMediaBuffer, mimetype: 'video/mp4', fileName: `${selectedMedia.title}.mp4`, caption: `𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗 වෙමින්🔍: ${selectedMedia.title}` };
          }
