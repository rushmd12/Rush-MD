const promote = async (m, gss) => {
  try {
    ...
    if (users.length === 0) {
      return m.reply("*⚙️ මේ command එක යොද ගැනීමෙන් වලකින්න*");
    }
    ...
    await gss.groupParticipantsUpdate(m.from, validUsers, 'promote')
      .then(() => {
        ...
        m.reply(`*Users ${promotedNames} promoted successfully in the group ⚙️මේ command එක යොද ගැනීමෙන් වලකින්න ${groupMetadata.subject}.*`);
      })
