const kick = async (m, gss) => {
  try {
    ...
    if (!botAdmin) return m.reply("*⚙️ BOT සදහා admin තිබිය යුතුයි*");
    ...
    if (users.length === 0) {
      return m.reply("*⚙️ PLEASE MENTION OR QUOTE A USER TO KICK*");
    }
    ...
    await gss.groupParticipantsUpdate(m.from, validUsers, 'remove')
      .then(() => {
        ...
        m.reply(`*USERS ${kickedNames} KICKED SUCCESSFULLY FROM THE GROUP✔️ ${groupMetadata.subject}*`);
      })
